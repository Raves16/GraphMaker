...
int draw_quadratic_function(std::string function)
{
	...
	if (success) {
		...
		WriteToFile(pngdata, "Quadratic/"+function+".png");
...
int draw_modulus_function(std::string function)
{
	_mkdir("./Modulus");
	bool success;
	StringReference* errorMessage = CreateStringReferenceLengthValue(0, L' ');
	...
	for (int z = 0; z < 15; z++)
	{
		ysa[z] = calculate_modulus_function(xsa[z]);
	}
	...
	if (success) {
		std::vector<double>* pngdata = ConvertToPNG(imageReference->image);
		WriteToFile(pngdata, "Modulus/modulus.png");
		DeleteImage(imageReference->image);
	}
	else {
		std::cerr << "Error: ";

...
double* funcArray(double D, int i, std::string function)
{
	...
	if (D > 0)
	{
		if (get_x_zero_negative(function) > get_x_zero_positive(function))
		{
			...
			arr[7] = (get_x_zero_negative(function) + get_x(function)) / 2;
			arr[8] = get_x(function);
			arr[9] = (get_x_zero_positive(function) + get_x(function)) / 2;
...
double calculate_modulus_function(double x)
{
	double solution;
	if (x >= 0)
	{
...
void run()
{
	...
	else if (find_char(function, '|') != -1)
	{
		draw_modulus_function(function);
	}
}